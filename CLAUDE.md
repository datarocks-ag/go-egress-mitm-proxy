...
- Redaction is secure-by-default: built-in masked headers (`Authorization`, `Proxy-Authorization`, `Cookie`, `Set-Cookie`, plus the URL-bearing `Location`, `Content-Location`, `Referer`) always apply, and `redactURL` strips `user:password@`. **Bodies are never redacted** — `renderBody` takes no redactor; `redact_headers` extends the set, and a `-` prefix removes a built-in (`-location` un-masks redirect targets without disabling anything else); removing a credential-bearing default warns at startup, and removing a header that is not masked is a load error; `redact_query` masks query-string values and **defaults to true** (set `false` to opt out); `log_secrets: true` is the escape hatch that disables all redaction.
