...
`BLACK-LISTED-CONNECT` is recorded at the CONNECT stage when a blacklisted host is intercepted. Interception is preferred over refusing the tunnel because it lets the handler return a readable 403, but the denial is then only recorded if the client completes the TLS handshake with the proxy CA — a certificate-pinning SDK or a JVM with its own truststore never will. Counting the attempt at CONNECT keeps it observable either way, and the separate label stops it inflating the `BLACK-LISTED` count that `HandleRequest` records when the handshake does succeed.

A host that is both passthrough *and* blacklisted is refused outright, since a passthrough tunnel is never inspected; that case counts as `BLACK-LISTED`.
...
**Redaction is secure-by-default.** `Authorization`, `Proxy-Authorization`, `Cookie`, and `Set-Cookie` are always masked, as are the URL-bearing `Location`, `Content-Location` and `Referer` — a 302 in an OAuth flow routinely carries the authorization code in `Location`, and header values are masked by name, so `redact_query` (which only sees the request URL) never covered it; `redact_headers` extends the set, and an entry prefixed with `-` removes one of the built-ins — `redact_headers: ["-location"]` makes redirect targets visible while `Authorization` and `Cookie` stay masked. Removing a credential-bearing default (`Authorization`, `Proxy-Authorization`, `Cookie`, `Set-Cookie`) is allowed but logs a warning at startup, and removing a header that is not masked is a load error rather than a silent no-op. `redact_query` masks query-string values and **defaults to `true`** — set it to `false` to opt out. Set `log_secrets: true` to disable all redaction (use with care — these logs may be shipped/retained).
